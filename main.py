...
if __name__ == "__main__":
    ...
    try:
        ...
        username.send_keys("akshayd02")
        password.send_keys("Hello#123") #changed
        ...
        time.sleep(2)
        for i in df:
            print(f"Number of test cases in module number {i}",len(df[i]))
        report = pd.DataFrame(df)
        report.to_excel('Testing_Report.xlsx')
        driver.quit()
